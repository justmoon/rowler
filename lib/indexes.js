...
function writeIndex(db, tr, keyPath, id, value){
  ...
  tr.set(packedKey, util.packValue(subspace, value));
}
...
function _fetchIndex(db, iter){
  ...
  iter.forEach(function(kv, cb) {
    ...
    var obj = {_id: id};
    obj[keyPath[keyPath.length-3]] = util.unpackValue(db.subspace, kv.value);
    result[id] = obj;
    cb();
  }, function(err) {
    if(err){
...
function readIndexGreater(db, tr, keyPath){
  ...
  var begin = fdb.KeySelector.firstGreaterThan(db.subspace.pack(key)).next();
...
function readIndexGreaterOrEqual(db, tr, keyPath){
  ...
  var begin = fdb.KeySelector.firstGreaterThan(db.subspace.pack(key));
  var range = db.subspace.range(_.initial(key));
...
function readIndexLess(db, tr, keyPath){
  ...
  var begin = fdb.KeySelector.firstGreaterThan(db.subspace.pack(_.initial(key)))
  var end = fdb.KeySelector.lastLessOrEqual(db.subspace.pack(key));
...
function readIndexLessOrEqual(db, tr, keyPath){
  ...
  var begin = fdb.KeySelector.firstGreaterThan(db.subspace.pack(_.initial(key)))
  var end = fdb.KeySelector.lastLessOrEqual(db.subspace.pack(key)).next();
